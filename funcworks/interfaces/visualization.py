...
class ParseMatricesInputSpec(BaseInterfaceInputSpec):
    regressor_names = traits.List(desc='List of regressors of interest')
    confound_names = traits.List(desc='List of regressors of no interest')
    mat_file = traits.File(exists=True, desc='Matrix File produced by Generate Model')
    con_file = traits.File(exists=True, desc='Contrast File Produces by Generate Model')
    entities = traits.Dict(desc='Dictionary containing BIDS file entities')


class ParseMatricesOutputSpec(TraitedSpec):
    design_matrix = traits.Any(desc='Path to design matrix')
    design_plot = File(desc='SVG File containing the plotted Design Matrix')
    contrast_plot = File(desc='SVG File containing the plotted Contrast Matrix')
    correlation_plot = File(desc='SVG File containing the plotted Correlation Matrix')


class ParseMatrices(BaseInterface):

    input_spec = ParseMatricesInputSpec
    output_spec = ParseMatricesOutputSpec
    ...
    def _list_outputs(self):
        ...
        design_matrix, corr_matrix, contrast_matrix = \
            self._parse_matrices(regressor_names=self.inputs.regressor_names,
                                 confound_names=self.inputs.confound_names,
        ...
        corr_plot = self._plot_corr_matrix(corr_matrix=corr_matrix,
                                           entities=ents,
                                           path_pattern=image_pattern,
                                           regressor_names=self.inputs.regressor_names)
        ents.update({'suffix':'design'})
        design_path = Path.cwd() / build_path(ents, path_patterns=design_matrix_pattern)
    ...
    @staticmethod
    def _plot_matrix(matrix, entities, path_pattern, suffix=None):
        ...
        matrix_path = Path.cwd() / build_path(entities, path_patterns=path_pattern)
        matrix_path.parent.mkdir(exist_ok=True, parents=True)
        plt.savefig(matrix_path, bbox_inches='tight')
        return matrix_path

    @staticmethod
    def _plot_corr_matrix(corr_matrix, entities, path_pattern, regressor_names):
        fig = plt.figure(figsize=(10, 10))
